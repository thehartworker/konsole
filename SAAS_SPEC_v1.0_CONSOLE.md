# SAAS Spec v1.0 — Intake-Konsole (Kern-Produkt)

**Stand:** 10. Juli 2026
**Autor:** Panel + Claude, für Bastian Scherbeck
**Status:** v1.0 nach Panel-Sprint, ready für morgigen Markier-Sprint
...
### 3.5 Modell-Wahl, Provider-Abstraktion und Fallbacks

**Provider-Abstraktion (grundlegend).** Alle LLM-Aufrufe der Konsole und der Backend-Handler laufen über eine interne Abstraktions-Schicht (`packages/classifier/src/providers/` und analog für Handler). Diese Schicht definiert ein einheitliches Interface für LLM-Aufrufe (Prompt-Input, Streaming-Output, Retry-Semantik, Token-Zählung, Fehler-Behandlung).

In v1 gibt es nur eine Implementation dieses Interfaces: den Anthropic-Provider. Aber die Struktur erlaubt es, in v1.1 oder v1.2 einen zweiten Provider (Mistral EU, Aleph Alpha, oder ein anderer) hinzuzufügen, ohne die Handler-Logik zu ändern.

Aufwand für den Abstraktions-Bau in v1: 1 bis 2 Tage in Woche 2 oder 3.

**Warum das wichtig ist:**
- Kein Provider-Lock-in im Code
- Compliance-Argument gegenüber Kunden, die einen EU-basierten LLM-Provider verlangen ("wir sind unabhängig, wir können jederzeit auf einen EU-Provider wechseln")
- Ausfall-Sicherheit gegen einzelne Provider-Störungen in v1.1 nachziehbar

**Modell-Wahl innerhalb Anthropic für v1:**
...
MAX_TOKENS 16000 wegen Denken-vor-Antwort-Semantik neuerer Modelle.
...
**Für Backend-Handler:** pro Handler individuell wählbar, weil manche Handler tieferes Reasoning brauchen (Opus-Klasse) und andere schnellere Antworten (Haiku-Klasse).

**Fallback bei LLM-Ausfall:** der Vorgang wird als "Klassifikation nicht möglich, manuelle Bearbeitung nötig" in die Konsole gelegt, mit dem rohen Nachrichten-Text und der Kanal-Herkunft. Kein Datenverlust, aber Latenz-Verzicht in Ausnahmefällen. Sobald ein zweiter Provider in v1.1 verfügbar ist, wird bei Ausfall des primären Providers automatisch auf den sekundären umgeschaltet.
...
**Stufe 1: Shadow-Mode.** Nichts wird automatisch versendet. Alle Antworten warten auf manuelle Freigabe. **Das ist der Default für alle neuen Kunden, mindestens für die ersten 4 Wochen produktiven Betriebs.**

**Stufe 2: Auto-Bestätigung mit Kurz-Kontrolle.** Antwort-Nachrichten (Empfangsbestätigungen ohne Rückfragen) gehen automatisch raus, aber die Beraterin bekommt eine Kurz-Benachrichtigung und kann innerhalb einer konfigurierbaren Toleranz-Zeit (Default 60 Sekunden) zurückrollen. Aktivierung nur durch die Chef-Rolle nach mindestens 4 Wochen Stufe-1-Betrieb, mit expliziter Kunden-spezifischer Aktion und Anzeige der Konsequenzen im UI (kein Häkchen in einem Einstellungs-Menü).

**Stufe 3: Voll-Autonomie.** Antworten gehen ohne Kurz-Kontrolle raus. Nur sensitive Vorgänge werden immer an den Menschen geleitet. Aktivierung nur durch die Chef-Rolle nach mindestens 12 Wochen kombiniertem Stufe-1-und-Stufe-2-Betrieb pro Kunde.

Wichtig: Stufe 2 und Stufe 3 sind niemals der Default. Der Default ist immer Stufe 1. Der Aufstieg ist bewusster Vertrauens-Aufbau, keine Voreinstellung.
