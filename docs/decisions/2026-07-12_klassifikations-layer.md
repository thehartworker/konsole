...
1. **Token-Verbrauch-Speicherort** (siehe oben, Punkt 4): Tabelle `llm_nutzung` wie vorgeschlagen, oder Alternative? — **Entschieden in Issue #30 (Teil 2), siehe Nachtrag unten.**

## Nachtrag Teil 2 (Persistenz), 2026-07-12 (Issue #30)

**Zu 4 (Token-Verbrauch-Speicherort), jetzt entschieden:** Tabelle `llm_nutzung` wie vorgeschlagen, mit zwei Abweichungen vom obigen Vorschlag, beide direkt aus dem Issue-#30-Auftrag übernommen:
- Spalte heißt `handler_slug` statt `zweck`, konsistent zur bestehenden Spalte `handler_aufrufe.handler_slug`.
- `handler_slug` ist `text` und **nullable**, nicht `text NOT NULL`. Grund: `'klassifikation'` ist kein gültiger Wert des Postgres-Enums `handler_slug` aus `20260711130000_enums_und_basistabellen.sql` (das Enum listet nur `W1_...` bis `W6_...`). Die Spalte bleibt deshalb bewusst `text` statt des Enums (ein zusätzlicher Enum-Wert würde Klassifikation und Backend-Handler-Semantik vermischen), und nullable, weil künftige LLM-Calls ohne festen Zweck-Slug denkbar sind. Migration: `supabase/migrations/20260712080000_llm_nutzung.sql`.

**Persistenz-Architektur (Aufgabe A):** neues Paket `packages/persistence` (`@konsole/persistence`), analog zur bestehenden Trennung `packages/llm` (Provider-Abstraktion) / `packages/classifier` (Prompt- und Validierungs-Logik, kennt weder Supabase noch Service-Role). `packages/persistence` kennt beides bewusst: es ist die in der Decision oben ("Grenze Klassifikation/Handler-Auslösung") beschriebene Teil-2-Schicht, die `KlassifikationsErgebnis` entgegennimmt und über eine `KlassifikationsRepository`-Schnittstelle (Interface-Pattern wie `LLMProvider`) persistiert. Die Supabase-Implementierung dieser Schnittstelle nutzt den Service-Role-Client (RLS-Bypass, wie in `docs/decisions/2026-07-10_rls-policies.md`, "Konsequenzen" für den Klassifikations-Ingest-Pfad festgehalten). Die Interface-Trennung erlaubt Unit-Tests der Status-Übergänge und Fehlerpfade mit einer In-Memory-Fake-Implementierung, ohne eine echte Postgres-Instanz zu brauchen (die bleibt der pgTAP-Suite vorbehalten, siehe `supabase/tests/`).

**Atomarität ("kein halber Vorgang"):** `persistiereKlassifikation()` schreibt `anliegen` in einem einzigen Batch-Insert (atomar als eine SQL-Anweisung), danach den `audit_log`-Eintrag, danach erst die `vorgaenge`-Zeile mit `klassifikation_status = 'done'`. Schlägt ein Schritt nach dem `anliegen`-Insert fehl, werden die bereits eingefügten `anliegen`-Zeilen kompensierend gelöscht und `vorgaenge.klassifikation_status` wird auf `'failed'` gesetzt. Das ist keine echte Datenbank-Transaktion über mehrere REST-Aufrufe hinweg (das bräuchte eine serverseitige Postgres-Funktion/RPC), sondern ein Kompensations-Muster (Saga). Bewusst gewählt, weil die Geschäftslogik (Routing-Auflösung, Statusübergänge, Fehlerpfade) in TypeScript testbar und lesbar bleiben soll, konsistent mit dem Rest des Repositories, das komplexe Logik in `packages/*` hält und PL/pgSQL auf Trigger/RLS-Helfer beschränkt. Kandidat für eine spätere Härtung zu einer echten RPC-Transaktion, falls sich Teil-Ausfälle in der Praxis zeigen.

**`zustaendige_nutzer_id`-Auflösung aus `routing.person_slug`:** `nutzer` hat keine `slug`-Spalte (siehe `docs/decisions/2026-07-10_datenmodell.md`, die Auflösung explizit als Klassifikations-Layer-Logik statt Datenmodell-Scope einordnet). Teil 2 löst `person_slug` deshalb gegen einen aus `nutzer.name` deterministisch abgeleiteten Slug auf (kleingeschrieben, Leerzeichen zu Unterstrich, Umlaute transliteriert `ä→ae` etc.), gescoped auf die Agentur. Kein Treffer → `NULL`, wie im Auftrag gefordert ("falls vorhanden, sonst NULL"). Das ist eine pragmatische Implementierung ohne zusätzliche Migration, keine wörtliche Spec-Vorgabe (die Spec zeigt nur ein Beispiel, `"julia_schmidt"`). Sollte sich in der Praxis zeigen, dass Namens-Kollisionen (zwei Nutzer mit demselben Slug) real vorkommen, ist eine echte `nutzer.slug`-Spalte mit Unique-Constraint der nächste Schritt. Dieselbe Auflösung läuft auch für `routing.verteiler` (§3.4 liefert dort ebenfalls person_slugs, nicht UUIDs) -- `vorgaenge.routing_verteiler` ist laut Datenmodell-Decision `uuid[]`, ein roher Slug-String-Insert würde am Spaltentyp scheitern. Nicht auflösbare Verteiler-Slugs werden verworfen statt den ganzen Vorgang scheitern zu lassen, weil `verteiler` kein Pflichtfeld mit garantierter Auflösbarkeit ist.

**Shadow-Mode-Durchsetzung (Aufgabe C):** `packages/persistence` hat, wie schon `packages/classifier` in Teil 1, strukturell keinen Code-Pfad, der einen Handler aufruft oder `antwort_nachricht` versendet — Persistenz ist in v1 das Ende der Kette (siehe Decision oben). Zusätzlich zur strukturellen Abwesenheit gibt es eine explizite Guard-Funktion `autonomieErlaubtAutomatischenVersand(autonomieLevel)`, die für Stufe 1 immer `false` liefert und für Stufe 2/3 `true` (in v1 ungenutzt, da kein Versand-Code existiert, aber als Vertrag für den späteren Versand-Schritt dokumentiert, siehe Decision oben, "Shadow-Mode-Durchsetzung"). Ein Test beweist, dass `persistiereKlassifikation()` bei Stufe 1 keinerlei Handler-/Versand-Funktion aufruft.

**Retry-Wrapper-Konsistenz (Aufgabe E, cross-referenziert):** beim Bau von Teil 2 hat sich gezeigt, dass die in "Konsequenzen" oben beschriebene Unerreichbarkeits-Lücke im Retry-Muster nicht nur `AGENTS.md` §7.4 betraf, sondern auch `packages/llm/src/retry.ts` selbst (wörtliche Übernahme, siehe dort) — die Behauptung im Issue-#30-Auftrag, `retry.ts` sei bereits korrekt, war nicht zutreffend. Beides ist jetzt gefixt: der letzte Versuch wirft bei anhaltendem Rate-Limit jetzt zuverlässig `Rate-Limit persistent nach N Retries` statt des rohen HTTP-Fehlers. Siehe `AGENTS.md` §7.4, `packages/llm/src/retry.ts`, `packages/llm/tests/retry.test.ts`.
