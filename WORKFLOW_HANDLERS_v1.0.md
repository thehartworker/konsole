# Workflow-Handler v1.0 — die sechs Backend-Fähigkeiten der Konsole
...
**Wichtig zur Sprach-Regel:** Bei DACH-Agenturen sind die internen Felder immer in Deutsch, weil sie Kommunikation *innerhalb* der Agentur sind (Berater:in liest, Etatdirektor:in reviewt, Kund:in gibt frei — alle deutsch-sprachig). Nur die Absender-gerichteten Texte (rueckfrage_nachricht, antwort_nachricht) folgen der Sprache der eingegangenen Anfrage. Konfiguration pro Agentur nur für die Absender-Ausgabe-Sprache, nicht für die internen Felder.

Die sechs Felder:
- `what_were_doing` (Deutsch als Standard bei DACH-Agenturen, Narrativ, was passiert)
...
**Version:** v0.1
**Status:** in Spec, "myconvento in klein" plus "myconvento-Aufsatz"-Konzept
**v1-Launch:** nein, v1.1 als Beta

### Zwei Betriebsarten in v1

W4 unterstützt zwei Betriebsarten, die derselbe Nutzer je nach Situation aufruft. Die Betriebsart wird pro Aufruf gewählt, nicht pro Agentur fest zugewiesen.

**Betriebsart 1: Öffentliche Recherche (Default).** Der Nutzer gibt ein Thema ein, W4 sucht mit Google News API, RSS-Feeds und LinkedIn-öffentlichen-Autoren-Profilen. Grundgesamtheit sind alle Journalist:innen, die in den letzten 6 bis 12 Monaten öffentlich zum Thema publiziert haben. Ergebnis: 20 bis 30 Treffer.

Passend für Agenturen, die keinen bestehenden Journalisten-Datenbestand haben oder eine breite Aufnahme neuer Kontakte suchen.

**Betriebsart 2: Datenbestand-Aufsatz.** Die Agentur hat einen bestehenden Verteiler-Bestand (aus pressbase, Meltwater, Landau, oder selbst gepflegter Excel/CSV). Sie importiert diesen Bestand einmalig in die Konsole. W4 nutzt diesen Bestand als Grundgesamtheit und macht die Intelligence-Schicht darüber: welche der eingespielten Personen sind für dein aktuelles Thema relevant, was haben sie zuletzt geschrieben, wie sprichst du sie an.

Passend für Agenturen mit bestehendem Journalisten-CRM, die keinen zweiten Bestand aufbauen wollen, sondern ihren bestehenden mit KI-Intelligence anreichern.

**Positionierung:** "Wir helfen dir bei der Journalisten-Recherche, egal ob du bei Null anfängst oder auf deinem bestehenden Verteiler aufsetzt."
...
- Manueller Aufruf durch Beraterin: "Wer schreibt gerade über [Thema]" (mit Wahl der Betriebsart)
...
interface W4Input {
  thema: string;
  betriebsart: "oeffentlich" | "bestand_aufsatz";
  kunde_kontext: {
    kunde_slug: string;
    branche: string;
    region_relevanz: string[];
  };
  bestand_ref: string | null;  // Referenz auf importierten Bestand, nur bei betriebsart="bestand_aufsatz"
  ziel: "recherche_liste" | "ansprache_generierung" | "beides";
  ansprache_anlass: string | null;  // z.B. "Pressemitteilung zu [Thema]"
  ausschluss_medien: string[];
...
Drei Stages, unterschiedlich je nach Betriebsart in Stage 1:

**Stage 1a (Betriebsart öffentlich): Publikations-Recherche.** Suche über Google News API und optional gecrawltes Corpus (falls Agentur eigenes hat) und LinkedIn öffentliche Autoren-Profile. Zeitraum letzte 6 bis 12 Monate. Filterung nach Thema, Region, Branche.

**Stage 1b (Betriebsart Bestand-Aufsatz): Bestand-Filter.** Grundgesamtheit ist der importierte Agentur-Bestand. Filterung: welche Personen im Bestand sind für das aktuelle Thema thematisch relevant. Anreicherung mit letzten öffentlichen Publikationen dieser Personen (Google News plus RSS auf den bekannten Personen-Namen).

**Stage 2: Autoren-Ranking.** Claude Sonnet-Klasse. Für jede:n Autor:in: Extrahierung des Publikations-Profils (welche Themen, welche Winkel, welche Häufigkeit). Ranking nach Relevanz für das Thema.

**Stage 3: Personalisierte Ansprache.** Bei `ziel = "ansprache_generierung"` oder `"beides"`: pro Top-Autor:in eine personalisierte Ansprache-Zeile basierend auf einem oder zwei konkreten letzten Artikeln. Kein generisches "Da Sie kürzlich...", sondern echter Bezug.

### Output-Kontrakt

Bleibt strukturell gleich für beide Betriebsarten:

```typescript
interface W4Output {
  betriebsart_verwendet: "oeffentlich" | "bestand_aufsatz";
  recherche_ergebnis: {
    ...
    autoren: Array<{
      ...
      kontakt_moeglich: {
        methode: string | null;  // "impressum", "linkedin", "twitter", "bestand_kontaktdaten"
        hinweis: string;
      };
...
### Import-Mechanik für Bestand-Aufsatz

Import ist ein separater Onboarding-Schritt pro Agentur, kein Bestandteil des Handler-Aufrufs.

Unterstützte Import-Formate in v1: CSV und Excel (xlsx). Pflicht-Spalten: Name, Medium, E-Mail-Adresse oder LinkedIn-URL. Optional: Ressort, Themen-Schwerpunkte, Notizen.

Nicht in v1: direkte API-Anbindungen an pressbase, Meltwater, Landau. Sinnvoll ab v1.1, wenn ein konkreter Kunde einen konkreten Anbieter fordert.

### Failure-Fallbacks

- Wenige Treffer bei öffentlicher Recherche: klare Ansage "Wenige aktuelle Artikel zum Thema, Empfehlung breiter suchen oder Bestand-Aufsatz-Modus prüfen"
- Bestand leer bei Betriebsart Bestand-Aufsatz: Handler lehnt ab mit klarer Meldung "Bestand nicht gefunden, bitte importieren"
...
- Beide Betriebsarten funktional
- Nur öffentliche Recherche-Daten (keine gekauften Datenbanken in v1)
- Fokus DACH-Medien
- Keine Automatisierung des Versands
- Keine direkten Journalisten-Datenbank-API-Anbindungen (nur CSV/Excel-Import)

### DSGVO-Besonderheit

Journalist:innen sind publizierende Personen und stehen dadurch anders in der DSGVO als Privatpersonen. Aber: das Ranking auf Basis ihres Publikations-Verhaltens ist rechtlich als "berechtigtes Interesse" der Agentur einzuordnen und muss in der AVV klar benannt sein. Der Handler nutzt nur öffentlich zugängliche Daten. Beim Bestand-Aufsatz-Modus: die rechtliche Verantwortung für den importierten Bestand liegt bei der Agentur (die musste ihn ohnehin rechtlich sauber beschaffen), die SaaS-Gesellschaft ist reiner Auftragsverarbeiter.
